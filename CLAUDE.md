...
L2~L7 헤더를 자유롭게 조립한 패킷을 **고속(PF당 100G × 4 = 총 400G)** 으로 전송하고,
업로드한 PCAP을 그대로 재현해 비정상 handshake를 포함한 프로토콜 동작을
**테스트·검증**하는 리눅스 기반 도구.

**기준 프레임은 64B**(확정). 400G = **595.2 Mpps**, PF 1개(100G) = 148.8 Mpps.
현재 검증기는 40G 상한이라 목표의 1/10 스케일이지만 **PF 4개 = 파드 4개** 구조가
동일하다 → [장비 2단 구성](#장비--검증기와-목표기-2단-구성).
...
| 계층 | 기술 | 역할 |
|---|---|---|
| 데이터플레인 | **C + DPDK** | PF당 100G line rate 커널 우회 송수신. 순수 C (C++ 의존성 없음) |
...
| 모드 | 목적 | 성능 | 레이어 |
|---|---|---|---|
| **A. Stateless 고속** | 대량 트래픽, L2~L4 이상동작 | PF당 100G line rate | L2~L4 |
...
- 베어메탈: Intel PMD (100G E810=ice, 40G X710/XL710=i40e)
