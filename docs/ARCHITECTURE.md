...
> **`StartScenario` 는 세 모드로 분기한다** (ipc_server): `pcap_path` 있으면
> 모드 C(리플레이), `handshake` 있으면 모드 B(세션), 아니면 모드 A(블라스트).
> PCAP 은 ⑤ 채널(공유 볼륨)을 경유하므로 이 요청에는 **경로 문자열만** 실린다.
...
- **Phase 0~6 완료, 전부 검증기 실물 실측** (세부는 [CLAUDE.md](../CLAUDE.md#현재-상태)).
- **채널 ①~⑤ 전부 실사용 중**이다. worker lcore(모드 A) · RX 폴링 lcore(모드 B/판정)
  가 `mir_stats`(①) 에 쓰고, RX lcore 가 이벤트를 `rte_ring`(②) 에 넣는다.
- **`StartScenario` 는 모드 A/B/C 로 실제 송신·세션·리플레이를 시작**한다.
- 텔레메트리의 포트별 수치는 **NIC 하드웨어 카운터**에서 온다 — 데이터플레인이
  "몇 개 보냈다"는 것과 NIC 이 실제로 내보낸 수를 대조할 수 있다.
- **남은 것**: Phase 7(400G 튜닝, 목표기 E810 필요·미도입) · Phase 8(클라우드).
